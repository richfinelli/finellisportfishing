 ...
 ***  Visit www.tectite.com for free <a href="http://www.tectite.com/">html form script</a>.
 *
...
$TARGET_EMAIL = array("^rich.finelli@yahoo\.com$");

            /* Help: http://www.tectite.com/fmdoc/def_alert.php */
define("DEF_ALERT","rich_ard_f@yahoo.com");
...
define('MSG_NEED_THIS_FORM',137);   // Multi-page forms require "formmail/this_form" field...
...
function LoadLanguageFile()
{
    ...
    AddIncludePath();
    if (!@include("formmail/language.inc.php"))
        @include("formmail/language.inc");
    ResetIncludePath();
...
function LoadBuiltinLanguage()
{
    ...
    $aMessages[MSG_MULTIFORM_FAILED] = 'Failed to process multi-page form template "$NAME"';
        ...
        // when a multi-page form does not specify the "formmail/this_form" field.
        // Parameters:
        //  none
    $aMessages[MSG_NEED_THIS_FORM] = 'Multi-page forms require "formmail/this_form" field';
...
function SetRealDocumentRoot()
{
    ...
    //
if ($HOOK_DIR !== "")
    if (!@include("formmail/$HOOK_DIR/fmhookpreinit.inc.php"))
		@include("formmail/$HOOK_DIR/fmhookpreinit.inc");
...
function OrderHeaders($a_headers)
{
    ...
    // This simply places a backslash in front every double-quote.
...
function DoMail($s_to,$s_subject,$s_mesg,$a_headers,$s_options)
{
    ...
    {
        require_once("formmail/Mail.php");
...
function MakeFromLine($s_email,$s_name)
{
    ...
    case "AddrSpecName":
        ...
        if (!empty($s_name))
            $s_line .= "(".SafeHeaderComment($s_name).")";
        break;
    case "NameAddrSpec":
                //
    ...
    case "QuotedNameRouteAddr":
                ...
                //
        if (!empty($s_name))
            $s_line .= '"'.SafeHeaderQString($s_name).'" ';
        if (!empty($s_email))
            $s_line .= "<".SafeHeaderEmail($s_email).">";
        break;
    case "NameRouteAddr":
...
if (isset($aGetVars["return"]) && is_numeric($aGetVars["return"]))
{
    ...
    if ($bAdvTemplates)
    {
        ...
        if (!include_once("formmail/$MODULEDIR/$FMCOMPUTE"))
    ...
    //
if ($HOOK_DIR !== "")
    if (!@include("formmail/$HOOK_DIR/fmhookpostinit.inc.php"))
		@include("formmail/$HOOK_DIR/fmhookpostinit.inc");
...
if ($SPECIAL_VALUES["file_names"] !== "")
    ...
    //
if ($HOOK_DIR !== "")
    if (!@include("formmail/$HOOK_DIR/fmhookload.inc.php"))
		@include("formmail/$HOOK_DIR/fmhookload.inc");
...
if ($FORM_INI_FILE !== "")
{
    ProcessFormIniFile($FORM_INI_FILE);
        ...
        //
    if ($HOOK_DIR !== "")
        if (!@include("formmail/$HOOK_DIR/fmhookinifile.inc.php"))
		    @include("formmail/$HOOK_DIR/fmhookinifile.inc");
}
...
if (!empty($SPECIAL_VALUES["fmcompute"]) || $bAdvTemplates)
{
    ...
    if (!empty($SPECIAL_VALUES["fmcompute"]))
    {
        ...
        if (!empty($GEOIP_LIC))
        {
            ...
            if (!include_once("formmail/$MODULEDIR/$FMGEOIP"))
...
if ($bIsGetMethod && count($aFormVars) == 0)
{
    ...
    //
if ($HOOK_DIR !== "")
    if (!@include("formmail/$HOOK_DIR/fmhookprechecks.inc.php"))
		@include("formmail/$HOOK_DIR/fmhookprechecks.inc");
...
if (isset($SPECIAL_VALUES["imgverify"]) && $SPECIAL_VALUES["imgverify"] !== "")
{
    ...
    //
if ($HOOK_DIR !== "")
    if (!@include("formmail/$HOOK_DIR/fmhookchecks.inc.php"))
		@include("formmail/$HOOK_DIR/fmhookchecks.inc");
...
if (!empty($SPECIAL_VALUES["fmmodules"]))
{
    ...
    foreach ($aModuleList as $sModule)
        if (!include_once("formmail/$MODULEDIR/$sModule"))
            Error("load_module",GetMessage(MSG_LOAD_MODULE,
...
if (!empty($SPECIAL_VALUES["fmcompute"]))
{
    ...
    Compute($aFieldOrder,$aCleanedValues,$aRawDataValues,$aAllRawValues);
        ...
        //
    if ($HOOK_DIR !== "")
        if (!@include("formmail/$HOOK_DIR/fmhookcompute.inc.php"))
		    @include("formmail/$HOOK_DIR/fmhookcompute.inc");
}
...
MultiFormLogic();
    ...
    //
if ($HOOK_DIR !== "")
    if (!@include("formmail/$HOOK_DIR/fmhookmulti.inc.php"))
		@include("formmail/$HOOK_DIR/fmhookmulti.inc");
                        ...
                        !empty($SPECIAL_VALUES["csvfile"]))
{
        ...
        //
    if ($HOOK_DIR !== "")
        if (!@include("formmail/$HOOK_DIR/fmhookprecsv.inc.php"))
            @include("formmail/$HOOK_DIR/fmhookprecsv.inc");
    WriteCSVFile($CSVDIR."/".basename($SPECIAL_VALUES["csvfile"]),$aAllRawValues);
        //
        // Hook system: after writing CSV file
        //
    if ($HOOK_DIR !== "")
        if (!@include("formmail/$HOOK_DIR/fmhookpostcsv.inc.php"))
		    @include("formmail/$HOOK_DIR/fmhookpostcsv.inc");
    $bDoneSomething = true;
}
...
if (!empty($LOGDIR) && isset($SPECIAL_VALUES["logfile"]) && !empty($SPECIAL_VALUES["logfile"]))
{
        ...
        //
    if ($HOOK_DIR !== "")
        if (!@include("formmail/$HOOK_DIR/fmhookprelog.inc.php"))
		    @include("formmail/$HOOK_DIR/fmhookprelog.inc");
    WriteLog($LOGDIR."/".basename($SPECIAL_VALUES["logfile"]));
        //
        // Hook system: after writing log file
        //
    if ($HOOK_DIR !== "")
        if (!@include("formmail/$HOOK_DIR/fmhookpostlog.inc.php"))
		    @include("formmail/$HOOK_DIR/fmhookpostlog.inc");
    $bDoneSomething = true;
}
    ...
    !empty($SPECIAL_VALUES["crm_url"]) && !empty($SPECIAL_VALUES["crm_spec"]))
{
    ...
    if (!empty($sCRM))
    {
        $aCRMReturnData = array();
            ...
            //
        if ($HOOK_DIR !== "")
            if (!@include("formmail/$HOOK_DIR/fmhookprecrm.inc.php"))
		        @include("formmail/$HOOK_DIR/fmhookprecrm.inc");
        if (!SendToCRM($sCRM,$aCRMReturnData))
        {
            ...
            //
        if ($HOOK_DIR !== "")
            if (!@include("formmail/$HOOK_DIR/fmhookpostcrm.inc.php"))
                @include("formmail/$HOOK_DIR/fmhookpostcrm.inc");
        $bDoneSomething = true;
    }
}
...
else
{
        ...
        //
    if ($HOOK_DIR !== "")
        if (!@include("formmail/$HOOK_DIR/fmhookpreemail.inc.php"))
            @include("formmail/$HOOK_DIR/fmhookpreemail.inc");
    ...
    else
    {
        ...
        if (!SendResults($aFieldOrder,$aCleanedValues,$s_valid_recipients,$s_valid_cc,
            ...
            //
        if ($HOOK_DIR !== "")
            if (!@include("formmail/$HOOK_DIR/fmhookpostemail.inc.php"))
                @include("formmail/$HOOK_DIR/fmhookpostemail.inc");
    }
}
...
if (isset($SPECIAL_VALUES["arverify"]) && $SPECIAL_VALUES["arverify"] !== "")
{
    if (IsAROptionSet('HTMLTemplate') || IsAROptionSet('PlainTemplate'))
    {
        ...
        else
        {
                ...
                //
            if ($HOOK_DIR !== "")
                if (!@include("formmail/$HOOK_DIR/fmhookprearesp.inc.php"))
                    @include("formmail/$HOOK_DIR/fmhookprearesp.inc");
            ...
            else
            {
                WriteARLog($sAutoRespTo,$sAutoRespSubj,
                    ...
                    //
                if ($HOOK_DIR !== "")
                    if (!@include("formmail/$HOOK_DIR/fmhookpostaresp.inc.php"))
                        @include("formmail/$HOOK_DIR/fmhookpostaresp.inc");
            }
        }
    }
}
...
else
{
        ...
        //
    if ($HOOK_DIR !== "")
        if (!@include("formmail/$HOOK_DIR/fmhookprefinish.inc.php"))
            @include("formmail/$HOOK_DIR/fmhookprefinish.inc");
        //
    ...
    if (!$bGotGoodUrl)
    {
        if ($bGotGoodTemplate)
            ...
            //
        if ($HOOK_DIR !== "")
            if (!@include("formmail/$HOOK_DIR/fmhookpostfinish.inc.php"))
                @include("formmail/$HOOK_DIR/fmhookpostfinish.inc");
            //
